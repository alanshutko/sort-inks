...
struct SortInks: ParsableCommand {
    @Flag(name: .short, help: "Display bottles only.")
    var bottlesOnly = false

    @Argument(help: "The file to read.")
    var filename: String
}

let options = SortInks.parseOrExit()
...
while csv.next() != nil {
    ...
    if !options.bottlesOnly || csv["Type"] == "bottle" {
        colors.insert(color)
    }
}
...
var row_height = 1
if sorted_colors.count < 200 {
    row_height = (200 / sorted_colors.count)
}
